...
###########################   CREATE DOC FUNCTION   ############################
def create_doc_1_pdf(data: dict, user = '') -> None:
    document = Document()
    ...
    if data["supruga_brak_number"] != "" or data["child1_svidetelstvo_data"] != "":
        ...
        p7.add_run(
            "Подтверждаю достоверность документов, обосновывающих семейный статус."
        )
    ...
    try:
        ...
        pass
    document.save(f"./files/doc1/document_pdf{user}.docx")

    ###############################  CONVERT TO PDF  ###############################
    subprocess.run(
        ["doc2pdf", f"./files/doc1/document_pdf{user}.docx"],
    ...
    ###############################    RENAME FILE   ###############################
    subprocess.run(
        ["mv", f"./files/doc1/document_pdf{user}.pdf", f"./files/doc1/document{user}.pdf"],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
        encoding="utf-8",
    )
